...
client.on('message', async msg => {
    ...
    if (command === 'play') {
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            try {
                ...
                try {
                    ...
                    .setAuthor(`Please select a song`, "https://images-ext-1.discordapp.net/external/2fG56UtfyTSowWQ6HhhPIV9VrZoD_OcVdHVwWpu6rIY/https/rythmbot.co/rythm.gif")
                    .setColor("RANDOM")
                    .addField("Send message number between 1 - 10 to select a song!" ,`${videos.map(video2 => `**${++index} -** ${video2.title}`).join('\n')}`)
                    .setFooter(`You have half a minute to send a message.`)
        ...
        if (!serverQueue) return msg.channel.send('Nothing playing in this server');
